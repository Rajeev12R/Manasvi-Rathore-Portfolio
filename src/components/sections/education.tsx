      ...
      <div className="container px-4 md:px-6">
        <div className="flex flex-col items-center justify-center space-y-4 text-center mb-12">
          ...
          <p className="max-w-[900px] text-muted-foreground md:text-xl lg:text-base xl:text-xl slide-up font-body" style={{ animationDelay: '0.2s' }}>
            My academic journey, building a solid foundation in journalism, communication, and essential skills.
          </p>
        </div>
...
function EducationCard({ education, index }: EducationCardProps) {
  ...
  return (
    <Card className="shadow-sm hover:shadow-lg transform hover:-translate-y-1 transition-all duration-300 flex flex-col slide-up border-secondary h-full bg-card" style={{ animationDelay: `${delay}s` }}>
      <CardHeader className="pb-4">
         <div className="flex items-center gap-3 mb-2">
           {education.isUniversity ? (
            ...
            <CardTitle className="text-lg font-heading font-semibold text-foreground leading-tight">{education.degree}</CardTitle>
         </div>
        <CardDescription className="text-primary font-medium pt-1 font-body">{education.institution}</CardDescription>
      </CardHeader>
          ...
          <ul className="space-y-1 pt-2 text-muted-foreground font-body text-sm">
            {education.details.map((detail, i) => (
              <li key={i} className="flex items-center gap-2 ml-1">
                 {detail.includes('Percentage') && <Percent className="h-4 w-4 text-primary/80 flex-shrink-0" />}
                 {!detail.includes('Percentage') && <span className="w-4 h-4 flex-shrink-0"></span>} {/* Spacer for alignment */}
                <span>{detail}</span>
              </li>
            ))}
          </ul>
