...
import { ExternalLink, FileText, Video, MicVocal, ArrowRight, Newspaper, Radio } from 'lucide-react'; // Added Newspaper, Radio
...
const workItems: WorkItem[] = [
    ...
    },
    {
      ...
      description: "Contributed a child's voice-over for a segment during the 'CT Topper Event' on My FM radio.",
    ...
    },
    {
      ...
      description: 'Authored news articles and produced video segments covering a recent local community initiative.',
    ...
    },
    {
      ...
      description: 'Hosted and produced a podcast episode discussing contemporary ethical challenges in media.',
    ...
    },
    {
      ...
      description: 'Conducted and edited a video interview profiling a local emerging artist and their work.',
...
export function WorkGrid() {
  ...
  return (
    <section id="work-grid" className="w-full py-16 md:py-24 lg:py-32 bg-background"> {/* Changed bg color */}
      <div className="container px-4 md:px-6">
        {/* Filter Buttons */}
        {categories.length > 1 && (
            <div className="flex justify-center flex-wrap gap-3 mb-10 slide-up" style={{ animationDelay: '0.1s' }}>
              {categories.map(category => (
                <Button
                  key={category}
                  variant={filter === category ? 'default' : 'outline'}
                  onClick={() => setFilter(category)}
                  className={`transition-all duration-300 ease-out font-body rounded-full px-5 py-2 text-sm md:text-base ${filter === category ? 'bg-primary text-primary-foreground shadow-lg scale-105' : 'border-primary/50 text-primary hover:bg-primary/10 hover:scale-105 hover:shadow-sm'}`}
                >
                  {category}
                </Button>
              ))}
        ...
        {/* Work Grid */}
        <div className={`grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3 transition-all duration-500 ease-out`}>
          {filteredWork.length > 0 ? (
            filteredWork.map((item, index) => (
              <WorkCard key={item.id} item={item} index={index} />
            ))
          ) : (
             <p className="col-span-full text-center text-muted-foreground italic font-body py-12 text-lg slide-up" style={{ animationDelay: '0.3s' }}>
...
function WorkCard({ item, index }: WorkCardProps) {
  ...
  return (
    <Card className="overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col slide-up border-border/50 group h-full bg-card hover:border-primary/60 transform hover:-translate-y-1.5" style={{ animationDelay: `${delay}s` }}>
      ...
      <CardContent className="p-5 flex-grow flex flex-col"> {/* Slightly reduced padding */}
        <CardTitle className="text-xl mb-2 text-foreground font-heading group-hover:text-primary transition-colors line-clamp-2">{item.title}</CardTitle>
        <CardDescription className="text-muted-foreground font-body mb-4 flex-grow line-clamp-3">{item.description}</CardDescription>
      </CardContent>
       <CardFooter className="p-5 pt-0 mt-auto border-t border-border/30"> {/* Added border */}
         <Button variant="link" asChild className="text-primary hover:underline p-0 h-auto font-body group/link text-sm font-medium">
           <Link href={item.link}>
             {item.category === 'Article' ? 'Read Article' :
              item.category === 'Video' ? 'Watch Video' :
              item.category === 'Voice' ? 'Listen Now' :
              item.category === 'Report' ? 'View Report' :
              item.category === 'Radio' ? 'Listen Segment' :
              'View Project'}
