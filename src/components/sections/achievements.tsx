...
const achievements: AchievementItem[] = [
  {
    ...
    description: "Earned a certificate, enhancing practical journalism knowledge.",
    date: "Jun 21-23, 2024",
  },
  {
    icon: Film,
    title: "\"Wetlands for Life\" Workshop",
    description: "Attended and created a short film, developing environmental storytelling skills.",
    date: "Feb 3-5, 2024",
  },
   // Example of adding MY FM voice-over if framed as an achievement/recognition
    {
      ...
      description: "Provided a child's voice-over for a radio segment during the 'CT Topper Event'.",
      date: "Jun 2024",
    },
];
      ...
      <div className="container px-4 md:px-6">
        <div className="flex flex-col items-center justify-center space-y-4 text-center mb-12">
          ...
          <p className="max-w-[900px] text-muted-foreground md:text-xl lg:text-base xl:text-xl slide-up font-body" style={{ animationDelay: '0.2s' }}>
            Highlighting key accomplishments and participation in relevant workshops and events that showcase skill development and initiative.
          </p>
        </div>
...
function AchievementCard({ achievement, index }: AchievementCardProps) {
  ...
  return (
    <Card className="shadow-sm hover:shadow-lg transition-all duration-300 flex flex-col items-start gap-4 p-6 slide-up border-secondary h-full bg-card transform hover:-translate-y-1" style={{ animationDelay: `${delay}s` }}>
       <achievement.icon className="h-10 w-10 text-primary mb-3 flex-shrink-0" />
       <div className="flex-grow flex flex-col">
        <CardTitle className="text-lg font-heading font-semibold text-foreground mb-2">{achievement.title}</CardTitle>
        <p className="text-sm text-muted-foreground font-body mb-4 flex-grow">{achievement.description}</p>
